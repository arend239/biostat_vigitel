...
gt_tab_logit <- tab_logit %>%
  ...
  gt::gt() %>%
  gt::tab_header(title="Razões de Chances (OR) – Consumo de Ultraprocessados e Doenças Crônicas")
...
![](images/agbrasil.png){.absolute   top="80" left="0"}
...
## Exposição:
  ...
  - Variáveis Relacionadas: consumo de refrigerante, embutidos, biscoitos recheados, fast food...
...
## Confundidores candidatos:
  ...
  - Comportamentais: **atividade física**

## Análise de prevalência global{.smaller}
...
🔹 **RP** é a medida de associação recomendada para estudos transversais com desfechos não raros (obesidade). Usando **OR** superestimaríamos a magnitude de associação.
...
![](images/modelo-1.png){.absolute   top="80" left="0" width="450"}

![](images/modelo-2.png){.absolute   top="80" right="0" width="450"}
